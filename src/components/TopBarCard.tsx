...
export const TopBarCard: React.FC<TopBarCardType> = ({ onClick, icon, name, borderColor,
    ...
    return (
        <Link href={linkURL} onClick={onClick}>
            <div style={{ borderColor: borderColor, backgroundColor: backgroundColor, borderRadius: borderRadius }} className={`flex gap-x-2 py-2.5 px-5 border-[0.5px] rounded-xl font-medium` +
                ...
                <p style={{ color: textColor }} className={"inline-block text-[14px] align-middle mx-auto"}>
                    {name}
                </p>
            </div>
        </Link>
